...
2. Run `docker run --name vpn -d --net host -v /path/to/profile.ovpn:/opt/openvpn/profile.ovpn:ro --device /dev/net/tun:/dev/net/tun --cap-add NET_ADMIN ghcr.io/dangh/aws-vpn-client-docker:latest`
   1. Run `docker logs -f vpn` to grab the login link
   2. After logging in, you can safely exit the log tail with `Ctrl-C`
3. Enjoy

If you are using a fork, the image path will be `ghcr.io/<owner>/aws-vpn-client-docker:<tag>`.
...
The workflow publishes to `ghcr.io/${{ github.repository }}`. A push to `master` publishes the `:master` image tag. A push of a Git tag like `v1.2.3` publishes both `:v1.2.3` and `:latest`. Pull requests run the same multi-arch build without pushing.
